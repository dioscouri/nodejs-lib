...
class MongooseModel extends Model {
    ...
    registerSchema (schemaObject, listName) {
        ...
        this.schema.post('save', instance => {
            ...
            if (this._validationPending.indexOf(instance._id.toString()) > 0) {
                console.log('Item validation is pending: ' + instance._id.toString());
                return; // Validation is pending
            }

            console.log(this._validationPending);

            console.log('Item validation queued: ' + instance._id.toString());
            this._validationPending.push(instance._id.toString());

            setTimeout(() => {
                // Use timeout for validation
                this.validateInstance(instance, () => {
                    this._validationPending = this._validationPending.filter(item => {
                        return item != instance._id.toString();
                    });
                    console.log('Item validation finished: ' + instance._id.toString());
                });
            }, 5000);
        });
    }
