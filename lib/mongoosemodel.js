...
class MongooseModel extends Model {
    ...
    registerSchema (schemaObject, listName) {
        ...
        this.schema.post('save', instance => {
            ...
            if (this._validationPending.indexOf(instance.id) > -1) {
                return; // Validation is pending
            }

            this._validationPending.push(instance.id);

            setTimeout(() => {
                // Use timeout for validation
                this.validateInstance(instance, () => {
                    this._validationPending = this._validationPending.filter(item => {
     ...
     * @param {Object} sorting
     */
    getStreamFiltered(filters, populations, sorting) {
        ...
        // Mongoose filter object
        let mongoFilters = this.prepareMongoFilters(locals);

        // Mongoose sorting object
        let mongoSort = this.prepareMongoSort(locals);
        
        return this.fetchStreamFiltered(populations, mongoFilters, mongoSort);
    }
    ...
    prepareMongoFilters (locals) {
        ...
        if (typeof this.addCustomFilters === 'function' && locals.filters.customFilters) {

            mongoFilters = this.addCustomFilters(mongoFilters, locals.filters.customFilters);
        }
