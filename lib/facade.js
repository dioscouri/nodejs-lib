...
class ApplicationFacade extends events.EventEmitter {
    ...
    loadApplications (configName) {
        ...
        if (this._appsConfig != null && this._appsConfig.applications != null && this._appsConfig.applications.length > 0) {
            for (var i = 0; i < this._appsConfig.applications.length; i++) {
                ...
                var appInstance = new ClassDefinition();

                this._modules[appName] = appInstance;
                this[appName] = appInstance;

                // Initializing Application
                if (appInstance.init != null && !appInstance.isAlreadyInitialized) {
                    appInstance.init();
    ...
    run () {
        ...
        ApplicationFacade.instance.server.application.use(Express.static('public'));
